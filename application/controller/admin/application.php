<?php
class c_application extends action{
	
	//游戏
	function a_do(){
		$fuhao='?';
		$num=10;
		$url='/admin/application.do';
		$page = I('G.page', 1);
		$bnum=($page-1)*$num;
		$whereArr[] = array('category','1');
		$totalprolist = D("application")->getList($whereArr);
		$total=count($totalprolist);
		$pages = H("page")->newpage($total, $num, $page, $url,$fuhao);
		$sql = "SELECT * FROM `a_application` ORDER BY `datetime` DESC limit ".($page-1)*$num.",".$num;
		$prolist = D('application')->getAll($sql);
		$data = array(
			'total' =>$total,
			'pages' => $pages,
			'prolist' => $prolist
	...
	function a_addgame(){
		...
		if($post['form_t']=='add' && $post['submit']){//添加图片提交操作
			...
			$this->display('admin/addgame_form');
		}
	}
	//添加类型
